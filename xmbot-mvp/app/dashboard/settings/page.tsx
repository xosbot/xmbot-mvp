...
import { Loader2, Save, Bot, Shield, Brain, MessageSquare, LinkIcon, Unlink } from "lucide-react"
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/engine?path=/api/config/" + session.user.id)
      ...
      .then((data) => {
        if (!data) return
        setBroker(data.broker?.broker || "paper")
        setSymbol(data.broker?.symbol || "XAUUSD")
        setMaxDailyLoss(String(data.risk?.max_daily_loss ?? 500))
        setMaxDrawdown(String(data.risk?.max_drawdown_percent ?? 20))
        setMaxPositionSize(String(data.risk?.max_position_size ?? 0.5))
        setDefaultStopLoss(String(data.risk?.default_stop_loss ?? 30))
        setEnableAI(data.enable_ai_analysis ?? true)
