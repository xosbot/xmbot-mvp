        ...
        <StaggerChildren className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6" staggerDelay={0.1}>
          {features.filter(f => f.featured).map((f) => (
            <StaggerItem key={f.title}>
              <Spotlight className="rounded-2xl" fill={f.color === "emerald" ? "rgba(16, 185, 129, 0.08)" : "rgba(139, 92, 246, 0.08)"}>
                <GlareCard className="p-8 h-full">
                  <div className={`w-14 h-14 rounded-xl flex items-center justify-center mb-6 ${
                    f.color === "emerald" ? "bg-emerald-500/10" : "bg-violet-500/10"
                  }`}>
                    <f.icon className={`h-7 w-7 ${f.color === "emerald" ? "text-emerald-400" : "text-violet-400"}`} />
                  </div>
